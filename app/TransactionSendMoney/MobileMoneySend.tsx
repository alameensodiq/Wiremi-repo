...
import { MobileTransferInstitution } from "@/Store/Apis/MobileTransferInstitution";
...
const MobileMoneySend = () => {
  ...
  const onChange = (name: string, value: any) => {
    console.log(value);
    setmobilemoneydetails((prevState) => ({
      ...prevState,
      [name]: value
    }));
  };
  ...
  const {
    mobiletransferinstitution,
    authenticatingmobiletransferinstitution,
  ...
  const { mobiletransfer, authenticatingmobiletransfer, errorsmobiletransfer } =
    useAppSelector((state) => state.mobiletransfer);
    ...
    if (bank?.code && mobilemoneydetails?.account_number?.length >= 12) {
      dispatch(
        AccountName({
          ...
          setShow: setShow4,
          bankCode: bank?.code,
          accountNumber: mobilemoneydetails?.account_number
        })
      );
    }
  }, [bank?.code, mobilemoneydetails?.account_number]);
  ...
  useEffect(() => {
    if (pin?.join("").length === 6) {
      ...
      dispatch(
        MobileTransfer({
          amount: Number(mobilemoneydetails?.amount),
          bank_code: bank?.code,
          account_number: mobilemoneydetails?.account_number,
          reason: mobilemoneydetails?.reason,
          country: country,
          router: router.push,
          name: bank?.name,
          pin: pin.join(""),
  ...
  const data3 = mobiletransferinstitution?.data;
            ...
            >
              ...
              >
                <View className="bg-white w-[70%] h-[30%] rounded-[10px] flex-col items-center justify-evenly py-3">
                  ...
                  {errorsmobiletransferinstitution?.error &&
                    typeof errorsmobiletransferinstitution.error === "object" &&
                    !Array.isArray(errorsmobiletransferinstitution.error) &&
                    Object.keys(errorsmobiletransferinstitution.error).map(
                      (key, index) => (
                        <Text key={index}>
                          {key}:{" "}
                          {Array.isArray(
                            errorsmobiletransferinstitution.error[key]
                          )
                            ? errorsmobiletransferinstitution.error[key].join(
                                ", "
                  ...
                  {errorsmobiletransferinstitution?.error &&
                    typeof errorsmobiletransferinstitution.error !==
                      "object" && (
                      <Text className="mb-3">
                        {errorsmobiletransferinstitution.error}
                      </Text>
            ...
            >
              ...
              >
                <View className="bg-white w-[70%] h-[30%] rounded-[10px] flex-col items-center justify-evenly py-3">
                  ...
                  {errorsmobiletransfer?.error &&
                    typeof errorsmobiletransfer.error === "object" &&
                    !Array.isArray(errorsmobiletransfer.error) &&
                    Object.keys(errorsmobiletransfer.error).map(
                      (key, index) => (
                        <Text key={index}>
                          {key}:{" "}
                          {Array.isArray(errorsmobiletransfer.error[key])
                            ? errorsmobiletransfer.error[key].join(", ") // Handle arrays by joining the elements
                            : errorsmobiletransfer.error[key]}{" "}
                        </Text>
                      )
                    )}
                  {errorsmobiletransfer?.error &&
                    typeof errorsmobiletransfer.error !== "object" && (
                      <Text className="mb-3">{errorsmobiletransfer.error}</Text>
                    )}
                  <ShortBlueButton
                    title="Close"
                    onPress={() => setIsVisible5(false)}
...
export default MobileMoneySend;
