...
import { MobileWithdrawInstitution } from "@/Store/Apis/MobileWithdrawInstitution";
import { clearStatemobilewithdrawinstitution } from "@/Store/Reducers/MobileWithdrawInstitution";
import { MobileWithdraws } from "@/Store/Apis/MobileWithdraws";
...
const WithdrawMobileMoney = () => {
  ...
  const [mobilemoneywithdrawdetails, setmobilemoneywithdrawdetails] = useState({
    amount: "",
    account_number: "",
    reason: ""
  });
  ...
  const onChange = (name: string, value: any) => {
    console.log(value);
    setmobilemoneywithdrawdetails((prevState) => ({
      ...prevState,
      [name]: value
    }));
  };
  ...
  const {
    mobilewithdrawinstitution,
    authenticatingmobilewithdrawinstitution,
    errorsmobilewithdrawinstitution
  } = useAppSelector((state) => state.mobilewithdrawinstitution);

  console.log(mobilewithdrawinstitution);
  ...
  const {
    mobilewithdraws,
    authenticatingmobilewithdraws,
    errorsmobilewithdraws
  } = useAppSelector((state) => state.mobilewithdraws);
  console.log(mobilewithdraws);

  useEffect(() => {
    dispatch(clearStatesummary());
    dispatch(clearStateaccountname());
    dispatch(clearStatemobilewithdrawinstitution());
  ...
  useEffect(() => {
    if (
      bank?.code &&
      mobilemoneywithdrawdetails?.account_number?.length >= 10
    ) {
      dispatch(
        AccountName({
          router: router.push,
          ...
          setShow: setShow4,
          bankCode: bank?.code,
          accountNumber: mobilemoneywithdrawdetails?.account_number
        })
      );
    }
  }, [bank?.code, mobilemoneywithdrawdetails?.account_number]);
  ...
  useEffect(() => {
    if (pin?.join("").length === 6) {
      ...
      dispatch(
        MobileWithdraws({
          amount: Number(mobilemoneywithdrawdetails?.amount),
          bank_code: bank?.code,
          account_number: mobilemoneywithdrawdetails?.account_number,
          reason: mobilemoneywithdrawdetails?.reason,
          country: country,
          router: router.push,
          name: bank?.name,
          pin: pin.join(""),
  ...
  useEffect(() => {
    if (mobilewithdraws?.status) {
      setPin(Array(6).fill(""));
      router.replace(
        `/More/Withdraw/WithdrawMobileMoneyReceipt?amount=${mobilewithdraws?.data?.amount}&fee=${mobilewithdraws?.data?.fee}&currency=${mobilewithdraws?.data?.currency}&receivername=${mobilewithdraws?.data?.counterparty?.account_name}&receiverbank=${mobilewithdraws?.data?.counterparty?.bank_name}&receivernumber=${mobilewithdraws?.data?.counterparty?.account_number}&type=${mobilewithdraws?.data?.type}&date=${mobilewithdraws?.data?.updated_at}&status=${mobilewithdraws?.data?.status}&reference=${mobilewithdraws?.data?.reference}&reason=${mobilewithdraws?.data?.reason}`
      );
    }
  }, [mobilewithdraws?.status]);
  ...
  const data3 = mobilewithdrawinstitution?.data;
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <View className="bg-white w-[70%] h-[30%] rounded-[10px] flex-col items-center justify-evenly py-3">
                  ...
                  {errorsmobilewithdrawinstitution?.error &&
                    typeof errorsmobilewithdrawinstitution.error === "object" &&
                    !Array.isArray(errorsmobilewithdrawinstitution.error) &&
                    Object.keys(errorsmobilewithdrawinstitution.error).map(
                      (key, index) => (
                        <Text key={index}>
                          {key}:{" "}
                          {Array.isArray(
                            errorsmobilewithdrawinstitution.error[key]
                          )
                            ? errorsmobilewithdrawinstitution.error[key].join(
                                ", "
                  ...
                  {errorsmobilewithdrawinstitution?.error &&
                    typeof errorsmobilewithdrawinstitution.error !==
                      "object" && (
                      <Text className="mb-3">
                        {errorsmobilewithdrawinstitution.error}
                      </Text>
            ...
            >
              ...
              >
                <View className="bg-white w-[70%] h-[30%] rounded-[10px] flex-col items-center justify-evenly py-3">
                  ...
                  {errorsmobilewithdraws?.error &&
                    typeof errorsmobilewithdraws.error === "object" &&
                    !Array.isArray(errorsmobilewithdraws.error) &&
                    Object.keys(errorsmobilewithdraws.error).map(
                      (key, index) => (
                        <Text key={index}>
                          {key}:{" "}
                          {Array.isArray(errorsmobilewithdraws.error[key])
                            ? errorsmobilewithdraws.error[key].join(", ") // Handle arrays by joining the elements
                            : errorsmobilewithdraws.error[key]}{" "}
                        </Text>
                      )
                    )}
                  {errorsmobilewithdraws?.error &&
            ...
            {authenticatingsummary || authenticatingmobilewithdraws ? (
              ...
              >
                <BlueSignInButton
                  title="Proceed"
                  // onPress={() =>
                  //   router.push("/More/Withdraw/WithdrawMobileMoneySummary")
                  // }
                  onPress={() => {
                    if (!mobilemoneywithdrawdetails?.reason) {
                      setShowreason(true);
                    } else {
                      dispatch(
                        Summary({
                          amount: mobilemoneywithdrawdetails?.amount,
                          country: country,
